...
class PlotCanvas(GraphWindow):
    ...
    def update_figure(self, data):
        ...
        tmax = (data.UV.time_array.max() - data.UV.time_array.min())
        tmax *= (24. * 60 * 60)
        extent = [data.UV.freq_array.min() * 1e-6,
                  data.UV.freq_array.max() * 1e-6, tmax, 0]
        vmax = 2.0*np.median(np.abs(calc))
        im = self.axes.imshow(
            np.abs(calc), aspect='auto', extent=extent, vmin=0, vmax=vmax)
...
class Main(QtWidgets.QMainWindow):
    ...
    def openFiles(self):
        ...
        file = QtWidgets.QFileDialog.getExistingDirectory(
            self, "Select UV file", "", options=options)
        ...
        self.bCBox.clear()
        self.pCBox.clear()
        # add the actual content of self.comboData
        self.aCBox.addItems(self.aData)
    ...
    def saveFile(self):
        ...
        fileName, _ = QtWidgets.QFileDialog.getSaveFileName(
            self, "Save UV file", "", "All Files (*);;UV Files (*.uv)",
            options=options)
        if fileName:
            print(fileName)
    ...
    def exportFile(self):
        ...
        options |= QtWidgets.QFileDialog.DontUseNativeDialog
        fileName, _ = QtWidgets.QFileDialog.getSaveFileName(
            self, "Export Labels", "", "All Files (*);;JSON Files (*.json)",
            options=options)
        if fileName:
            # handler for nonjson
    ...
    def initMenuBar(self):
        ...
        redoAct.setStatusTip('Repeat previous action')
    ...
    def initGraphWin(self):
        ...
        self.aCBox.currentIndexChanged.connect(
            lambda idx: self.selectCombo(idx, 0))
        self.bCBox = QtWidgets.QComboBox()
        self.bCBox.currentIndexChanged.connect(
            lambda idx: self.selectCombo(idx, 1))
        self.pCBox = QtWidgets.QComboBox()
        self.pCBox.currentIndexChanged.connect(
            lambda idx: self.selectCombo(idx, 2))

        self.keysEdit = QtWidgets.QLineEdit()
        self.notesEdit = QtWidgets.QTextEdit()

        self.saveBtn = QtWidgets.QPushButton("Save label")
        # connect to labelling to be disabled otherwise
        ...
        # space between this and keysEdit odd
